...
class GetStatisticsData:
    def __init__(self):
        ...
        self.result_excel_name = 'result' + \
            str(datetime.now().strftime('%Y%m%d%H')) + '.xlsx'

    def get_data(self, data_excel_name):
        remove_name_pd = pd.read_excel(
            self.remove_excel_name, sheet_name="Sheet1")
        # print remove_name_pd.values[0][0]
        for i in range(0, len(remove_name_pd)):
            self.remove_name.append(remove_name_pd.values[i][0])
        # for i in self.remove_name:
        #     print i
        # print "11111111111111111111111"
        company_dict_pd = pd.read_excel(
            self.company_dict_excel_name)
        self.company_dict = dict(
            zip(company_dict_pd[u'原名称'].values, company_dict_pd[u'输出名称'].values))
        company_dict_pd.rename(columns={'原名称': '所属服务站'}, inplace=True)
        out_day_pd = pd.read_excel(data_excel_name)
        # print len(out_day_pd)
        # 删除 跳过公司的数据
        for remove_name_one in self.remove_name:
<<<<<<< HEAD
            out_day_pd = out_day_pd[out_day_pd['单位名称'] != remove_name_one]
=======
            out_day_pd = out_day_pd[out_day_pd['所属服务站'] != remove_name_one]
>>>>>>> f619db7a7381ce9a4c068cde4e76e28165383780
        ...
        for i in range(0, len(re_sort)):
            index = '第' + str(i + 1) + '名'
            if len(re_sort.index[i]) == 3:  # 服务站名字为3个字的加1个空格
                file_handle.write(index + '\t' + re_sort.index[i] + '\t' + str(format(re_sort.values[i], '.2%'))
                                  + '\n')
            elif len(re_sort.index[i]) == 2:  # 服务站名字为2个字的加2个空格
                file_handle.write(
...
if __name__ == '__main__':
    ...
    file_handle.write('【昨日(5日)研判成绩排名】：' + '\n')
